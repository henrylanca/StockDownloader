...
using System.Net;
using static PeakCalculater.DownloadQuotes;
...
namespace PeakCalculater
{
    ...
    public class Historical
    {
        ...
        public static List<HistoryPrice> Get(string symbol, DateTime start, DateTime end, TimeFrame timeFrame = TimeFrame.Day)
        {
            List<HistoryPrice> HistoryPrices = new List<HistoryPrice>();

            try
            {
                string csvData = GetRaw(symbol, start, end, timeFrame);
                if (csvData != null)
        ...
        public static string GetRaw(string symbol, DateTime start, DateTime end, TimeFrame timeFrame)
        {
            ...
            try
            {
                ...
                if(timeFrame == TimeFrame.Week)
                    url = "https://query1.finance.yahoo.com/v7/finance/download/{0}?period1={1}&period2={2}&interval=1wk&events=history&crumb={3}";

                //if no token found, refresh it
                if (string.IsNullOrEmpty(Token.Cookie) | string.IsNullOrEmpty(Token.Crumb))
                {
                    if (!Token.Refresh(symbol))
                        return GetRaw(symbol, start, end, timeFrame);
                }
            ...
            catch (WebException webEx)
            {
                ...
                if (response.StatusCode == HttpStatusCode.Unauthorized)
                {
                    ...
                    return 
                        GetRaw(symbol, start, end, timeFrame);
                }
                else
                {
                    throw;
